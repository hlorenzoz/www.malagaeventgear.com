...
| **Estética y Visuales Premium**<br>Uso de variables CSS, glassmorphism, paleta de colores (DESIGN.md), y micro-animaciones. | `ui-ux-pro-max`<br>`frontend-design`<br>`high-end-visual-design`<br>`glassmorphism`<br>`minimalist-ui` | Wow-factor visual, glassmorphism sofisticado, paletas balanceadas e interacciones fluidas. |
...
| **SEO Técnico & Generativo**<br>Inyección de metadatos estructurados JSON-LD (Schema.org), optimización de imágenes (AVIF/WebP), prioridades de red, y Google Rich Results. | `debug-optimize-lcp`<br>`structured-data-schema`<br>`seo-audit`<br>`seo-geo`<br>`programmatic-seo`<br>`copywriting`<br>`landing-page-conversion-framework` | SEO Técnico y Local de alto rango, Schema Markup robusto, copywriting persuasivo y conversión. |
| **Rendimiento & Cloudflare**<br>Diagnóstico de cuellos de botella de JS, Edge Rendering, Wrangler y restricciones de Cloudflare Workers/Pages. | `performance-investigation`<br>`cloudflare-guard`<br>`cloudflare`<br>`cloudflare-deploy`<br>`workers-best-practices`<br>`wrangler`<br>`performance`<br>`web-perf`<br>`lighthouse` | Edge compatibility, wrangler config, optimizaciones críticas de carga y eliminación de scripts bloqueantes. |
...
| **Testing de Interfaces**<br>Validación visual, logs del navegador, y pruebas de integración locales con Playwright. | `webapp-testing`<br>`e2e-testing-patterns`<br>`tdd`<br>`test-driven-development` | Pruebas de integración automatizadas, desarrollo guiado por pruebas (TDD) y Playwright. |
| **Seguridad & Autenticación**<br>Protección contra ataques en la web, flujos de sesión del CRM y seguridad de base de datos. | `security-best-practices`<br>`better-auth-best-practices` | Prácticas sólidas de autenticación, escape de entradas de usuario y robustez de APIs. |
