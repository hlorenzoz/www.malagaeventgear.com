...
- **Mejoras de datos estructurados pendientes** (deferred — detectadas al verificar el JSON-LD de un post; el schema es válido, pero estos dos puntos son sistémicos y mejorables. NO implementar sin pedido explícito):
  1. **Nombre de la última miga (breadcrumb leaf)**: `buildBreadcrumbsSchema` en [`src/lib/utils/schema.ts`](file:///Users/hlorenzoz/databank/Development/%5BMEG%20-%20Malaga%20Event%20Gear%20%28malagaeventgear.com%29%5D/projects/website/src/lib/utils/schema.ts) deriva el `name` del segmento de URL (slug en Title Case), no del título real del post/página. Resultado feo en posts (ej. "News Malaga Event Gear Unveils New Rebranded Website"). Ideal: usar el título de la página cuando esté disponible.
  2. **`publisher` sin `@id`**: en `NewsArticle`/`Article` (`buildArticleSchema`), `publisher` redefine una `Organization` mínima en vez de referenciar el nodo canónico `{"@id": "…/#organization"}`. Consolidar el grafo de entidades referenciando por `@id` (igual que hacen `buildWebSiteSchema` / `buildServiceSchema`).
